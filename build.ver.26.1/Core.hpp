...
namespace Crypto_Impl{
    inline std::filesystem::path File_Crypto_SingleFile_Create(Crypto_Type crypto_type,const std::filesystem::path target_folder,const std::filesystem::path source_file){
        ...
        switch(crypto_type){
            ...
            default: target_file_name = source_file.filename().stem().string();
        }
    ...
    inline void Crypto_SFStateBoolState_Func_Clean(const Crypto_SFBoolState_Info_Clean SFBoolState_Info){
        auto rmEmptyDir = [](std::filesystem::path target_folder){
            if(File_Folder_CheckEmpty(target_folder))
                std::filesystem::remove(target_folder);
        };

        if(SFBoolState_Info.SFBoolState) {
            std::filesystem::remove(SFBoolState_Info.source_file);
            rmEmptyDir(SFBoolState_Info.source_file.parent_path());
        }
        else{
            std::filesystem::remove(SFBoolState_Info.target_file);
            rmEmptyDir(SFBoolState_Info.target_file.parent_path());
        }
    }
    ...
    inline void Crypto_File_Temporary(const Crypto_Operate_Info object_info){
        std::filesystem::path temporary_path;
        Crypto_Core_File(Crypto_Type::Dec,object_info,&temporary_path);

        Terminal_Template_Current_MountTemporary_NewTUI_Launch(File_Vim_TextCompiler,temporary_path);

        Crypto_Core_File(Crypto_Type::Enc,Crypto_Operate_Info{EncStore,temporary_path,object_info.password,true},nullptr);
    }
    inline void Crypto_Folder_File_Recursive(Crypto_Type crypto_type,const Crypto_Operate_Info object_info,std::filesystem::path* target_path_output) {
        if(!std::filesystem::is_directory(object_info.source_file)) {return;}
        File_StoreFolder_Create(object_info.target_folder);
        ...
        while(!Folder_cache.empty()) {
            ...
            for(const auto& entry: std::filesystem::directory_iterator(folder_current)){
                std::filesystem::path target_position = File_SourcePath_To_TargetPath_Convert(target_base_folder,object_info.source_file,entry.path());
                if(std::filesystem::is_directory(entry.path())){
                    ...
                    switch(crypto_type){
                        case Crypto_Type::Enc:{
                            ...
                            if(!SFBoolState) {
                                File_Folder_Clean(target_base_folder);
                                goto SFBoolState_Failed;
                            }
                            break;
                        }
                        default: {
        ...
        File_Folder_Clean(object_info.source_file);
    ...
    inline void Crypto_Folder_Temporary_Recursive(const Crypto_Operate_Info object_info){
        std::filesystem::path temporary_path;
        Crypto_Folder_File_Recursive(Crypto_Type::Dec,object_info,&temporary_path);

        Terminal_Template_Current_MountTemporary_NewTUI_Launch(Terminal_FileManager_Interface,temporary_path);

        Crypto_Folder_File_Recursive(Crypto_Type::Enc,Crypto_Operate_Info{EncStore,temporary_path,object_info.password,true},nullptr);
    }
};

inline void Crypto_Interface(const std::string crypto_type,const std::filesystem::path source_file,const std::string password) {
    auto Crypto_File_Formate_Select = [source_file](const Crypto_Type crypto_type,const Crypto_Impl::Crypto_Operate_Info& object_info) {
        if (std::filesystem::is_directory(source_file)) {
            Crypto_Impl::Crypto_Folder_File_Recursive(crypto_type,object_info,nullptr);
            return;
        }
        Crypto_Impl::Crypto_Core_File(crypto_type,object_info,nullptr);
    };
    if (crypto_type=="Enc") {
        Crypto_Impl::Crypto_Operate_Info object_info{EncStore,source_file,password,true};
        Crypto_File_Formate_Select(Crypto_Type::Enc,object_info);
